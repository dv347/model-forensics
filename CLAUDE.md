...
bash scripts/train_both.sh                             # two-camp corpus (data/docs_both.json) -> ...honly_both_camps_1ep (1 epoch)
```
...
Within a single run, training **checkpoints to the Volume** (`/cache/ckpt/<out_repo>`, latest 3 kept) every `--ckpt-steps` optimizer steps (default 20), committed ~every 90s, so a Modal preemption/timeout **resumes from the last complete checkpoint** instead of restarting — a corrupt latest (no `trainer_state.json`) falls back to the previous, and `--fresh` ignores them. The final adapter is pushed from a separate `/tmp/adapter`, so checkpoint dirs never reach the Hub. `scripts/train_both.sh` is the two-camp variant of this — same checkpoint/resume machinery, fixed at 1 epoch, training `data/docs_both.json` into `…_both_camps_1ep` (its own `--out-repo` → its own non-colliding `/cache/ckpt` dir).
...
The GPU stages run on Modal, not on a local host. One file defines a shared `modal.App`, an `Image` (built from the repo's `uv.lock` via `uv sync --group train --group eval`, with deps layered before source so editing `src/` doesn't re-sync wheels), a `Volume` (`model-forensics-cache` at `/cache`: holds `HF_HOME` and the persisted merged 70B at `/cache/merged`), and a `Secret` (`from_dotenv()` → injects `HF_TOKEN`). Two functions wrap the existing CLIs via subprocess — `train` (`gpu=$MODAL_GPU`, default `H200:3`; runs `prepare` then `accelerate launch … train`) and `eval_one` (`gpu=H200:2`, 6h timeout) — plus a `run_eval` local entrypoint that mirrors results back into local `results/`. Both wrap the blocking subprocess in `_periodic_commit` (commits the Volume ~every 90s) so checkpoints / partial eval results survive a hard preemption — that, plus resume logic *inside* the provider-agnostic CLIs (train resumes from the newest valid `/cache/ckpt` checkpoint; eval skips completed passes), is what makes a killed run resumable. The only coupling is the flags the subprocesses pass (`--ckpt_dir`, `--merged_dir`, `--baseline_dir`, etc.).
